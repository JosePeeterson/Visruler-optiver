...
export default {
  ...
  methods: {
    ...
    InitThresh () {
      ...
      const leftTrgl = d3v5.symbol().type(triangleLeft)
        .size(180)
      const rightTrgl = d3v5.symbol().type(triangleRight)
        .size(180)

      const head1 = slider.append('path')
        .attr('d', leftTrgl)
        .style('fill', 'black')
      	.style('stroke', 'none')
      ...
      const head2 = slider.append('path')
        .attr('d', rightTrgl)
        .style('fill', 'black')
      	.style('stroke', 'none')
          ...
          for (let i = startingPointRF; i < decisionsRFReadSort[k]; i++) {
            if (this.selectDecisionsTestLimitation.length == 0) {
              if (this.selectDecisions.length == 0) {
                ...
                const active = Object.values(RFStatsRead.predicted_value)[i]
          ...
          for (let i = (RFLength + startingPointAB); i < (RFLength + decisionsABReadSort[k]); i++) {
            if (this.selectDecisionsTestLimitation.length == 0) {
              if (this.selectDecisions.length == 0) {
                ...
                const active = Object.values(GBStatsRead.predicted_value)[reduceID]
