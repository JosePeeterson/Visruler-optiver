...
def xgb_preprocessing(XData,yData):

    XData1 = XData.copy()
    XData1['time_id'] = XData1['time_id'].astype('category')
    XData1['stock_id'] = XData1['stock_id'].astype('category')

    sample_size = int(len(XData1)/112)  # round down

    # drop categorical columns
    #XData.drop(columns=['stock_id'], inplace=True)

    train_ids = XData1.iloc[:int(sample_size*0.7)]['time_id']
    test_ids = XData1.iloc[int(sample_size*0.7):int(sample_size)]['time_id']

    X_train = XData1[XData1['time_id'].isin(train_ids)]
    yData = np.array(yData)
    y_train = yData[XData1['time_id'].isin(train_ids).values]
    ...
    X_test = XData1[XData1['time_id'].isin(test_ids)]
    y_test = list(yData[XData1['time_id'].isin(test_ids)])
...
def retrieveFileName():
    ...
    elif data['fileName'] == 'optiver_100':
        CollectionDB = mongo.db.optiver_100.find()
        target_names.append('low_vol')
        target_names.append('med_vol')
        target_names.append('high_vol')
...
def executeSearch():
    ...
    if data['fileName'] == 'TitanicC' or data['fileName'] == 'CreditC':
        ...
        #X_train, X_test, y_train, y_test = train_test_split(XDataNorm, yData, test_size=0.1, random_state=RANDOM_SEED, shuffle=False)
        #XData = pd.read_csv('C:\Finance_projects\VisRuler-paper-version\VisRuler-paper-version\data\optiver_100.csv')
        X_train, X_test, y_train, y_test = xgb_preprocessing(XData, yData)
    ...
    # allParametersPerformancePerModel = resultsLocalRF + resultsLocalXGB
    allParametersPerformancePerModel = resultsLocalXGB
    jsonFileRF = []
...
def SendEachClassifiersPerformanceToVisualize():
    ...
    response = { 
        ...
        'X_train': json.dumps(X_train, cls=NumpyArrayEncoder), #X_train, # pd.DataFrame(X_train).astype(int).to_json(), # json.dumps(X_train, cls=NumpyArrayEncoder),
        'y_train': json.dumps(y_train, cls=NumpyArrayEncoder), 
        'X_test': json.dumps(X_test, cls=NumpyArrayEncoder), # X_test, # pd.DataFrame(X_test).astype(int).to_json() ,
...
def requestedNewModels():
    ...
    limitationXGB = RetrievedModel['XGBNewModels']
